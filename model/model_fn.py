...
def _encoder(inputs, num_channels,
    ...
    with tf.variable_scope("encoder"):
        for l in range(blocks):
            with tf.variable_scope('block_{}'.format(l + 1)):
                output = tf.layers.conv2d(output, filters=num_channels, kernel_size=kernel_size, padding='same')
                if use_batch_norm:
                    output = tf.layers.batch_normalization(output, training=is_training)
                output = tf.nn.relu(output)
                output = tf.layers.max_pooling2d(output, pool_size=pool_size, strides=pool_strides)
